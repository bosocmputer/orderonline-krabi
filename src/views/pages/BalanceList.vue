...
const quotationTotal = computed(() => quotationCart.value.reduce((s, i) => s + (parseFloat(i.price) || 0) * (parseInt(i.qty) || 0), 0));
...
function addToQuotationCart(headerRow, detail) {
    ...
    const existIdx = quotationCart.value.findIndex((i) => i.item_code === headerRow.item_code && i.wh_code === detail.warehouse && i.shelf_code === detail.location);
    if (existIdx >= 0) {
        quotationCart.value[existIdx].qty = (parseInt(quotationCart.value[existIdx].qty) + qty).toString();
    } else {
...
async function sendQuotationOrder() {
    ...
    try {
        ...
        const items = quotationCart.value.map((item) => {
            const sumAmount = (parseFloat(item.price || 0) * parseInt(item.qty || 0)).toString();
            return {
...
async function lazyLoadDetailPrices(itemCode) {
    ...
    try {
        ...
        const promises = details.map((d) => BalanceService.getBalanceDetailPrice(d.item_code, d.location || '', d.unit_code || '', custCode, '1').catch(() => null));
...
function onSort(event) {
    ...
    const sortMap = {
        ...
        balance_qty_current_year: 'balance_qty_current_year',
        balance_qty_other_year: 'balance_qty_other_year'
    };
    ...
    <div class="balance-list-page">
        ...
        <div class="card filter-section">
            ...
            <div class="filter-grid">
                ...
                <div class="filter-item filter-item-wide search-block">
                    ...
                    <div v-for="(field, index) in searchFields" :key="field.id" class="search-field-row">
                        ...
                        <InputText v-model="field.value" :placeholder="index === 0 ? 'รหัสสินค้า / ชื่อสินค้า' : 'เงื่อนไขเพิ่มเติม...'" @keyup="onSearchKeyup" class="search-input" />
            ...
            <div class="filter-actions">
                ...
                <Button label="ใบเสนอราคา" icon="pi pi-file-edit" severity="warning" :badge="quotationTotalItems > 0 ? String(quotationTotalItems) : undefined" badgeSeverity="danger" @click="quotationPanelVisible = true" />
            </div>
        </div>
        ...
        <div class="card table-section">
            <DataTable
                ...
                :rowsPerPageOptions="[30, 50, 100, 500, 1000, 2000, 5000]"
                @page="onPage"
                @sort="onSort"
                removableSort
            ...
            >
                ...
                <Column field="price_0" header="ราคาขาย 0%" sortable style="min-width: 120px; text-align: right" headerClass="header-align-right">
                    <template #body="{ data }">
                        <span>{{ parsePrice(data.price_0) }}</span>
                    </template>
                </Column>
                <Column field="price_9" header="ราคาขายสด" sortable style="min-width: 120px; text-align: right" headerClass="header-align-right">
                    <template #body="{ data }">
                        <span>{{ parsePrice(data.price_9) }}</span>
                    </template>
                </Column>

                <Column field="balance_qty_current_year" header="สต๊อกปีนี้" sortable style="min-width: 110px; text-align: center">
                    <template #body="{ data }">
                ...
                <template #expansion="{ data }">
                    <div class="expansion-content">
                        ...
                        <template v-else-if="expandedDetails[data.item_code] && expandedDetails[data.item_code].length > 0">
                            ...
                            <DataTable :value="expandedDetails[data.item_code]" class="detail-table" responsiveLayout="scroll" :rowStyle="getDetailRowStyle" :rowClass="(row) => (!isDetailOrderable(row) ? 'row-not-orderable' : '')">
                                        ...
                                        <div class="quotation-action-cell">
                                            <div class="qty-input-row">
                                                <Button
                                                    ...
                                                    @click="locationQuantities[getQtyKey(data.item_code, detail.warehouse, detail.location)] = Math.max(0, (locationQuantities[getQtyKey(data.item_code, detail.warehouse, detail.location)] || 0) - 1)"
                                                />
                                                <input
                                                    type="text"
                                                    class="qty-input"
                                            ...
                                            <Button label="ใส่ตะกร้า" icon="pi pi-file-edit" severity="warning" size="small" :disabled="getQty(data.item_code, detail.warehouse, detail.location) <= 0" @click="addToQuotationCart(data, detail)" />
                                        </div>
                                    </template>
                                </Column>
                            </DataTable>
                        ...
                        <div class="expansion-images">
                            ...
                            >
                                ...
                                <template #thumbnail="slotProps">
                                    <img :src="slotProps.item.thumbnailImageSrc" :alt="slotProps.item.alt" @error="$event.target.src = ProductService.getPlaceholderImage()" style="width: 50px; height: 50px; object-fit: contain; border-radius: 4px" />
                                </template>
                            </Galleria>
                        </div>
                    </div>
        ...
        <!-- Drawer -->
        <Drawer :visible="quotationPanelVisible" @update:visible="quotationPanelVisible = $event" position="right" :style="{ width: '420px' }">
            <template #header>
                <div style="display: flex; align-items: center; gap: 0.6rem">
                    <i class="pi pi-file-edit" style="font-size: 1.2rem; color: var(--orange-500)"></i>
                    <span style="font-weight: 700">ตะกร้าใบเสนอราคา</span>
                    <Tag v-if="quotationTotalItems > 0" :value="quotationTotalItems + ' รายการ'" severity="warning" />
                </div>
            </template>
            ...
            <div v-else class="quotation-item-list">
                <div v-for="(item, idx) in quotationCart" :key="idx" class="quotation-item-card">
                    <div class="quotation-item-header">
                        ...
                        <Button icon="pi pi-trash" text rounded severity="danger" size="small" @click="removeFromQuotationCart(idx)" />
                    </div>
                    ...
                    <div class="quotation-item-controls">
                        <div class="qty-input-row">
                            <Button icon="pi pi-minus" text rounded class="qty-btn" @click="updateQuotationQty(idx, parseInt(item.qty) - 1)" />
                            <input type="text" class="qty-input" :value="item.qty" @change="(e) => updateQuotationQty(idx, e.target.value)" style="width: 3rem" />
                            <Button icon="pi pi-plus" text rounded class="qty-btn" @click="updateQuotationQty(idx, parseInt(item.qty) + 1)" />
            ...
            <template #footer>
                <div class="quotation-footer">
                    <div class="quotation-footer-total">
                        <span>ยอดรวม:</span>
                        <span class="quotation-total-amount">{{ quotationTotal.toLocaleString('en-US', { minimumFractionDigits: 2 }) }} บาท</span>
                    </div>
                    <div class="quotation-footer-actions">
                        <Button label="ล้างตะกร้า" icon="pi pi-trash" severity="secondary" outlined @click="clearQuotationCart" :disabled="quotationCart.length === 0" />
                        <Button label="พิมพ์ใบเสนอราคา" icon="pi pi-print" severity="success" @click="printQuotation" :disabled="quotationCart.length === 0" />
                    </div>
                    <div class="quotation-footer-actions" style="margin-top: 0.5rem">
                        <Button label="ส่งใบเสนอราคา" icon="pi pi-send" severity="warning" @click="confirmSendQuotationOrder" :disabled="quotationCart.length === 0 || sendingQuotationOrder" :loading="sendingQuotationOrder" style="width: 100%" />
                    </div>
                </div>
            </template>
        </Drawer>
        ...
        <div class="print-only">
            ...
            <table class="print-items-table">
                ...
                <tfoot>
                    <tr>
                        <td colspan="8" style="text-align: right; font-weight: bold">ยอดรวมทั้งหมด:</td>
            ...
            <div class="print-footer">
                ...
                <p style="color: #999; font-size: 0.85rem">วันที่ .................................</p>
            </div>
        </div>
    </div>
</template>
