...
//        loadSearchItemList();
    ...
    $("#selTableRows").change(function () {
        PAGE_SIZE = $("#selTableRows").val();
        CURRENT_PAGE = 0
        loadBalanceList();
    });
...
function sortPrice() {
    shortIc = shortIc === 'down' ? 'up' : 'down';
    $('#shortPrice').attr("class", "fa fa-sort-" + shortIc);
    sortCol = 'price';
    loadBalanceList();
}
...
function sortQty() {
    shortIc = shortIc === 'down' ? 'up' : 'down';
    $('#shortQty').attr("class", "fa fa-sort-" + shortIc);
    sortCol = 'balance_qty';
    loadBalanceList();
}
...
function sortYear() {
    shortIc = shortIc === 'down' ? 'up' : 'down';
    $('#shortYear').attr("class", "fa fa-sort-" + shortIc);
    sortCol = 'year_weak';
    loadBalanceList();
}
...
function sortDay() {
    shortIc = shortIc === 'down' ? 'up' : 'down';
    $('#shortDay').attr("class", "fa fa-sort-" + shortIc);
    sortCol = 'update_date';
    loadBalanceList();
}
...
function loadBalanceList() {
    ...
    { // pagination
        ...
        data.offset = CURRENT_PAGE * PAGE_SIZE;
    ...
    $.ajax(subLink + restURL + "?action_name=loadBalanceList", {
        ...
        beforeSend: function () {
            $('#btnLoadBalanceLot').attr('disabled', true);
            $("#contentTableBox").show();
            $("#contentTableBox").find("#list").html("<tr><td colspan='6'><h5><span class='fa fa-refresh fa-spin'></span> กำลังโหลด...</h5></td></tr>");
        ...
        success: function (res) {
            if (res.success) {
                datax = res.data;
                renderBalanceList()
                $('#btnLoadBalanceLot').removeAttr('disabled');
...
function _calPrice(price_cal, priceCon) {
    var splitPriceCon = priceCon.split(',');
    var priceCal = 0;
    if (splitPriceCon.length > 0) {

        $.each(splitPriceCon, function (index, ele) {
            var findminus = -1;
            var findpercent = -1;
            findminus = ele.indexOf("-");
            findpercent = ele.indexOf("%");
            if (findminus >= 0) {
                if (findpercent >= 0) {
                    price_cal -= ((price_cal * parseFloat(ele.replace('-', '').replace('%'))) / 100);
                } else {
                    price_cal -= parseFloat(ele.replace('-', ''));
                }
            } else {
                if (findpercent >= 0) {
                    price_cal += ((price_cal * parseFloat(ele.replace('%'))) / 100);
                } else {
                    price_cal += parseFloat(ele);
                }
...
function _qtyShow(qty) {
    ...
    if (qty <= 0) {
        ...
        show_qty = '4+';
    } else if (qty > 12 && qty <= 20) {
        show_qty = '12+';
    } else if (qty > 20 && qty <= 40) {
        show_qty = '20+';
    } else if (qty > 40 && qty <= 100) {
        show_qty = '40+';
    } else if (qty > 100) {
        show_qty = '100+';
    } else {
...
function renderBalanceList() {
    var priceCon = $('#txtUpdatePrice').val();
    resHTML = "";
    resHTMLExport = "";
    ...
    if (datax.length > 0) {
        ...
        $.each(datax, function (key, obj) {
            ...
            var priceCon = $('#txtUpdatePrice').val();
            console.log(priceCon)
            var price_cal = parseFloat(obj.price.replace(',', '').trim());
            if (priceCon != '') {
                price_cal = _calPrice(price_cal, priceCon);
            }


            qty += parseInt(obj.balance_qty.replace(',', '').replace(',', '').replace(',', ''));
            if (_qtyShow(obj.balance_qty) != "0") {
                console.log(obj.update_date)
                resHTML += "<tr>";
                resHTML += "<td style='text-align: left;' class='vertical-center'><b>" + obj.item_code + "</b> ~ " + obj.item_name + "</td>";
                resHTML += "<td class='vertical-center'>" + addCommas(price_cal.toFixed(2)) + "</td>";
                resHTML += "<td class='vertical-center'>" + _qtyShow(obj.balance_qty.replace(',', '')) + " (" + obj.unit_code + ")</td>";
                resHTML += "<td class='vertical-center'>" + obj.year_weak + "</td>";
                resHTML += "<td class='vertical-center'>" + formatDate(obj.update_date) + "</td>";

                resHTMLExport += "<tr>";
                resHTMLExport += "<td style='text-align: left;' class='vertical-center'><b>" + obj.item_code + "</b> ~ " + obj.item_name + "</td>";
                resHTMLExport += "<td class='vertical-center'>" + addCommas(price_cal.toFixed(2)) + "</td>";
                resHTMLExport += "<td class='vertical-center'>" + _qtyShow(obj.balance_qty.replace(',', '')) + " (" + obj.unit_code + ")</td>";
                resHTMLExport += "<td class='vertical-center'>" + obj.year_weak + "</td><td></td> </tr>";
                resHTMLExport += "<td class='vertical-center'>" + formatDate(datenow) + "</td>";
                ...
                resHTML += "<td colspan='7'>";
                resHTML += "<div>";
                resHTML += "<table class='table' style='margin-bottom: 0;'>";
                resHTML += "<thead>";
                resHTML += "<tr style='background-color: #FDEBD0;'><td style='text-align: left;'>คลัง.</td><td>ที่เก็บ</td><td>จำนวน</td><td>ค้างส่ง</td><td>ขายได้</td></tr>";
                resHTML += "</thead>";
        ...
        if (_qtyShow(qty) != "0") {
            resHTML += "<tr style='background:yellow;'>";
            resHTML += "<td class='vertical-center' colspan='2' style='text-align:left;'>GrandTotal</td>";
            resHTML += "<td class='vertical-center'>" + _qtyShow(qty) + "</td>";
            resHTML += `<td class='vertical-center' colspan='3'></td>`;
            resHTML += "</tr>";

            resHTMLExport += "<tr style='background:yellow;'>";
            resHTMLExport += "<td class='vertical-center' colspan='2' style='text-align:left;'>GrandTotal</td>";
            resHTMLExport += "<td class='vertical-center'>" + _qtyShow(qty) + "</td>";
            resHTMLExport += `<td class='vertical-center' colspan='2'></td>`;
            resHTMLExport += "</tr>";

            resHTMLExport =
                    '    <table>  <thead>' +
                    '        <tr>' +
                    '            <th style="width: 45%">รหัสสินค้า ~ ชื่อสินค้า</th>' +
                    '            <th style="width: 15%">ราคา</th>' +
                    '            <th style="width: 10%">จำนวน </th>' +
                    '            <th style="width: 10%">จากปีเดือน</th>' +
                    '            <th style="width: 10%"></th>' +
                    '        </tr>' +
                    '    </thead>' +
                    '    <tbody id="list">' + resHTMLExport + '</tbody>  </table>';
        }
    } else {
        resHTML = "<tr><td colspan='6'><h5><span class='fa fa-close text-danger'></span> ไม่พบข้อมูล</h5></td></tr>";
    }

    $("#contentTableBox").find("#list").html(resHTML);
    // return resHTML;
}

function fnExcelReport()
{
    var tab_text = resHTMLExport;
...
function renderBalanceDetail(data) {
    ...
    if (data.length > 0) {
        $.each(data, function (key, obj) {
            ...
            backgroundColor = "";
            if (obj.warehouse == "KBG2" || obj.warehouse == "KBYT2" || obj.warehouse == "VLT2") {
                backgroundColor = "style='background:#00ff80'";
            }
            resHTML += "<tr " + backgroundColor + ">";
            ...
            if (obj.warehouse == "KBG2" || obj.warehouse == "KBYT2" || obj.warehouse == "VLT2") {
                resHTML += "<td class='vertical-center' style='background-color:#00ff80'>" + parseFloat(obj.overdue).toFixed(0) + "</td>";
            } else {
                resHTML += "<td class='vertical-center' style='background-color:#FFCC66'>" + parseFloat(obj.overdue).toFixed(0) + "</td>";
            }

            var sale_diff = parseFloat(obj.balance_qty) - parseFloat(obj.overdue);
            resHTML += "<td class='vertical-center'>" + sale_diff + "</td>";
            resHTML += "</tr>";
        });
    } else {
        resHTML = "<tr><td colspan='6'><h5><span class='fa fa-close text-danger'></span> ไม่พบข้อมูล</h5></td></tr>";
...
function createPagination(total_records) {
    ...
    var xTotalpage = parseInt(total_records / PAGE_SIZE);
    //var xPage = CURRENT_PAGE;

    $("#contentPaginationList").empty();
    var page = parseInt(CURRENT_PAGE) === 0 ? 0 : CURRENT_PAGE;
...
//    }
    ...
    if (parseInt(page) === parseInt(xTotalpage)) {
        ...
        objPagiList.find("#btn-pagi-last").attr('page-id', xTotalpage).addClass("btn-pagination");
    }

    objPagiList.find("#txt-pagination").val("หน้า " + (page + 1) + " ถึง " + (xTotalpage + 1));

    if ($('#selTableRows').val() == "99999999") {
        objPagiList.find("#btn-pagi-next").attr("page-id", -1).addClass("btn-pagination disabled");
        objPagiList.find("#btn-pagi-last").attr('page-id', -1).addClass("btn-pagination disabled");
        objPagiList.find("#btn-pagi-first").attr('page-number', -1).addClass("disabled");
        objPagiList.find("#btn-pagi-previous").attr("page-number", -1).addClass("disabled");
        objPagiList.find("#txt-pagination").val("หน้า 1 ถึง 1");
    }
    // return HTML
    return objPagiList;
}