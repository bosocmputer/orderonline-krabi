...
public class BalanceLot extends HttpServlet {
    ...
    private JSONObject loadBalanceList(JSONObject param) throws Exception {
        ...
        __strQuery.append("coalesce(( select ic_inventory_price.create_date_time_now from ic_inventory_price where ic_inventory_price.cust_group_1 = 'GR02' and ic_inventory_price.sale_type = 0 and ic_inventory_price.status = 1 and ic_inventory_price.price_type = 2 and ic_inventory_price.price_mode = 1 and current_date between ic_inventory_price.from_date and ic_inventory_price.to_date and ic_inventory_price.ic_code = ic_inventory.code limit 1), now()) as update_date, ");
        ...
        if (flag != 0) {
            ...
            __strQuery.append(" coalesce(( select ic_inventory_price.create_date_time_now from ic_inventory_price where ic_inventory_price.cust_group_1 = 'GR02' and ic_inventory_price.sale_type = 0 and ic_inventory_price.status = 1 and ic_inventory_price.price_type = 2 and ic_inventory_price.price_mode = 1 and current_date between ic_inventory_price.from_date and ic_inventory_price.to_date and ic_inventory_price.ic_code = stk.ic_code limit 1), now()) as update_date, ");
        ...
        while (__rsData.next()) {
            ...
            __objData.put("update_date", __rsData.getString("update_date"));
    ...
    private JSONObject loadBalanceDetail(JSONObject param) throws Exception {
        ...
        while (__rsData.next()) {
            ...
            String __strQuery2 = "select item_code, "
                    + " coalesce((select balance_qty from sml_ic_function_stock_balance_warehouse_location(current_date, '" + __strItemCode + "', '" + __rsData.getString("warehouse") + "', '" + __rsData.getString("location") + "') where item_code=ic_code),0) as overdue "
            ...
            while (__rsData2.next()) {
                __objData.put("overdue", __rsData2.getString("overdue"));
            }
            __jsResult.put(__objData);
        }
